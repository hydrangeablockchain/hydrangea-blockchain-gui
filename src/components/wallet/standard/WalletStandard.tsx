...
import {
  ...
  Fee,
  Form,
  TextField as HydrangeaTextField,
  AlertDialog,
  CopyToClipboard,
  Flex,
  Card,
...
import { /* mojo_to_hydrangea_string, */ hydrangea_to_mojo } from '../../../util/hydrangea';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_hydrangea_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
  ...
  return (
    <Card title={<Trans>Balance</Trans>}>
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the total amount of hydrangea in the blockchain at the current
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      openDialog(
        <AlertDialog>
          <Trans>
            Error: Cannot send hydrangea to coloured address. Please enter a hydrangea
            address.
          </Trans>
        </AlertDialog>,
      );
    ...
    const amountValue = Number.parseFloat(hydrangea_to_mojo(amount));
    const feeValue = Number.parseFloat(hydrangea_to_mojo(fee));
